...
def plot_histogram(datei, histogram_separate, histogram_spec, num_bins=50):
    with open (datei, "rb") as daten:
	sim_array = pickle.load(daten)
	print sim_array
	...
	#print sim_array[0][1], sim_array[1]
	    ...
	    print "erstelle separate histogramme"
            ...
            si = sl[0]
            n, bins, patches = plt.hist(si.times, num_bins, range = meine_range, normed=1, alpha=0.5 )
            x = np.arange (min(si.times), max(si.times), 100)
	    l, = plt.plot(x, scipy.stats.invgauss.pdf(x, si.mu, si.loc, si.scale), lines[0], alpha=0.6)
	    
	    si = sl[1]
            ax = figg.add_subplot(222)
            n, bins, patches = plt.hist(sl[1].times, num_bins, range = meine_range, normed=1, alpha=0.5 )
            x = np.arange (min(si.times),max(si.times), 100)
	    l, = plt.plot(x, scipy.stats.invgauss.pdf(x, si.mu, si.loc, si.scale), lines[1], alpha=0.6)
	    
	    si = sl[2]
            ax = figg.add_subplot(223)
            n, bins, patches = plt.hist(sl[2].times, num_bins, range = meine_range, normed=1, alpha=0.5 )
            x = np.arange (min(si.times), max(si.times), 100)
	    l, = plt.plot(x, scipy.stats.invgauss.pdf(x, si.mu, si.loc, si.scale), lines[2], alpha=0.6)
	    
	    si = sl[3]
            ax = figg.add_subplot(224)
            n, bins, patches = plt.hist(sl[3].times, num_bins, range = meine_range, normed=1, alpha=0.5 )
            x = np.arange (min(si.times), max(si.times), 100)
	    l, = plt.plot(x, scipy.stats.invgauss.pdf(x, si.mu, si.loc, si.scale), lines[3], alpha=0.6)
		...
		#print sa[0][0].mu
		l, = plt.plot(x, scipy.stats.invgauss.pdf(x, si.mu, si.loc, si.scale), lines[i], alpha=0.6)
