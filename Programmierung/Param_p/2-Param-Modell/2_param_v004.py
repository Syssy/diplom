...
def main():
    ...
    pkombis = [(0.9999, 0.9999),(0.99993, 0.99993),(0.9997, 0.9997),(0.9997, 0.99993)]
    ...
    for ps, pm in pkombis:
        ...
        #for j in range(len(pms)):
            #ps = pss[i]
            #pm = pms[j]
            logging.log(25, "ps, pm, %s %s", ps, pm)
            speichern = False
            try:
		...
		with open(filename, 'rb') as datei:
            ...
            if speichern:
		filename = 'v004/l'+str(length)+"/n"+str(number)+ '/Sim_'+ str(round(ps,6)) +'_' +  str(round(pm,6))+ ".p" 
                logging.log(20, "Speichern, %s", time.strftime("%d%b%Y_%H:%M:%S"))
    ...
    n, bins, patches = plt.hist(mySim.times, 50, alpha=0.5 )
    plt.show()
