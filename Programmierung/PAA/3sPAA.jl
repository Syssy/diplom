...
function cut_Distributions(distributions, index)
    ...
    if vorneschneiden
        ...
        #print (" shiftindex, indexneu: $index, laenge dist ")
        #println (length(distributions[1]))
    end
...
function waitingTimeForValue(params::Array{Float32, 2}, value, maxTime, num_states = 3)
    ...
    index = 1
    # Zeitverschiebung als Offset fuer den Peakbeginn
    zeitverschiebung = 0
    ...
    for i = 1:maxTime
        ...
        # oder wenn keine angekommen sind (else) 0 bzw offset erhoehen
...
function combineParams()
    # Benennung der Wahrscheinlichkeiten:
    # p plus Startzustand, Endzustand, s bei Liste
