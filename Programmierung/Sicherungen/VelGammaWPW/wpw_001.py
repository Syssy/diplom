...
class Teilchen:
    anz = 0
    def __init__(self, ort = 0, mobil = False, vel = 1.00001, gamma = 0.999999):
	self.ort = ort
	self.mobil = mobil
	self.vel = vel
	self.gamma = gamma
	...
	# Parameterwahl abhaengig vom akt Zustand TODO: Das kann doch weg, wenn ich auf self.p zugreife...
	#if self.mobil:
	#    p=pm
	#else:
	#    p=ps
	...
	zeitspanne = scipy.stats.geom.rvs(1-self.p)
    ...
    # TODO: Hier für die variablen Geschwindigkeiten & Parameter die Berechnung reinpacken
    def berechne_params(self, zeitraum, drift_vel = 10.00001, gamma = 0.999999):
	if self.mobil:
	    #TODO: Vel erhöhen, sinnvolles maß?
	    self.vel += zeitraum*(drift_vel+self.vel/100000)
	    self.p = gamma / self.vel
	   # print "vel", self.vel, 'p', self.p
	else:
	    self.vel = 1#vel 
	    self.p = gamma	
	return
   ...
   # print ereignisse, len(ereignisse)
	...
	if zeit not in ereignisse:
	   pass#print "lalallala"
		...
		teilchen.berechne_params(zeitpunkt)
