...
def main():
   ...
   # pkombis = [(3500, 0.996, 5000, 0.9985),(2500, 0.996, 5000, 0.9985),(1500, 0.996, 5000, 0.9985),(4500, 0.996, 5000, 0.9985)] 
    ...
    for a in range(90000, 1000000, 10):
	for b in np.arange (0.999, 0.9992, 0.001):
	    for c in range(10000, 11000, 500):
		for d in np.arange(0.001, 0.009, 0.0001):
...
#	print zz
    ...
    for veldivisor, ps, gammadivisor, gamma0 in zufallskombis:  
		...
		try:
		    ...
		    filename = 'l'+str(length)+"/n"+str(number)+'/Sim_'+ str(round(veldivisor,6)) +'_' + str(round(ps,6)) +'_' +  str(round(gammadivisor,6))+'_'+ str(round(gamma0,6)) + ".pp" 
		    #print filename
		    with open (filename, 'rb') as datei:
			mySim = pickle.load(datei)
			#print mySim.params
