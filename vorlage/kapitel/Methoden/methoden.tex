...
\todo{korrekte Schreibweise von Boolschen werten}
...
\begin{scope}[start chain =1 going right, node distance =-0.3mm]
	...
	\node [on chain=1, feld,minimum width=2cm] (p1) {$e_1$};
	\node [on chain=1, feld,minimum width=2cm] (p2) {$e_2$};
	\node [on chain=1, feld,minimum width=2cm] (p3) {$e_5$};
	\node [on chain=1, feld,minimum width=2cm] (p4) {$e_7$};
	\node [on chain=1, feld,minimum width=2cm] (p5) {$e_{12}$};
\end{scope}
...
\path [line width=1.4pt, line cap=rect, draw](p1) edge (1x1) ;
...
\path [line width=1.4pt, line cap=rect, draw](p3) edge (3x1) ;

\path [line width=1.4pt, line cap=rect, draw](p4) edge (4x1) ;
...
Wie bei der schrittweisen Teilchensimulation tritt auch beim PAA für das 3-Zustände Modell das Problem auf, dass nicht mehr nur Boolsche Werte  ausreichen, um den Zustand zu beschreiben. 
