...
Zum Einen scheint es in den vorliegenden Datensätzen einige Peaks zu geben, deren kombinierte Lage und Breite nicht mit dem Modell simulierbar sind. Alle Simulationen ergeben zumindest geringfügig breitere Peaks. Zum Anderen weisen fast keine der simulierten Peaks ein Tailing auf. 
...
\begin{figure}
\subfigure[Keine Übergänge]{\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
...
\subfigure[Zwischenzustand]{\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.2cm,
  thick,main node/.style={circle,fill=white!20,draw,font=\sffamily\small\bfseries}]
